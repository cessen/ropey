...
fuzz_target!(|data: (StartingText, Vec<Op>)| {
    let mut r = match data.0 {
        StartingText::Small => Rope::from_str(SMALL_TEXT),
        StartingText::Medium => Rope::from_str(&SMALL_TEXT.repeat(100)),
        StartingText::Custom(s) => Rope::from_str(s),
    };
