...
//! they do/don't promise before using them.
...
///
/// In all other cases, this function's return value is unspecified (may change
/// between non-breaking releases) and should not be relied on for program
/// correctness. An example of such a case: rope B is cloned from rope A, and
/// then the same edit is made to both ropes.  They have then both been modified
/// since cloning (not case 1), but they also compare equal (not case 2 either).
///
/// Runs in O(1) time.
...
/// circumstances.  On failure, returns `None`.
///
/// Success is guaranteed for `RopeSlice`s created from `Rope`s or from
/// `Rope`-derived `RopeSlice`s.  Whether this function succeeds on `RopeSlice`s
/// constructed in other ways (namely `From<&str>`) is unspecified (may change
/// between non-breaking releases) and should not be relied on for program
/// correctness.
///
