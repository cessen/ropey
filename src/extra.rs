...
/// same content.
///
/// The API promises of this function are very specific.  The following two
...
///
/// In all other cases, this function's return value may change between
/// non-breaking releases and should not be relied on for program correctness.
/// An example of such a case: rope B is cloned from rope A, and then the same
...
///
/// Success is guaranteed for `RopeSlice`s from `Rope`s.  It is unspecified
/// whether `RopeSlice`s constructed in other ways (e.g. `From<&str>`) will
/// succeed or fail, meaning that whether they succeed or fail may change
